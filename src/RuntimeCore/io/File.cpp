...
namespace Rut
{
	namespace FileX
	{
		...
		void SaveFileViaPath(const wchar_t* wpPath, void* pData, size_t nBytes)
		{
			...
			if (nBytes > MAXDWORD) { throw std::runtime_error("SaveFileViaPath: File Too Large!"); }
			...
			WriteFile(hfile, pData, static_cast<DWORD>(nBytes), NULL, NULL);
		...
		size_t GetModuleNameViaBase_Ptr(char* cpBuf, uintptr_t uiBase)
		{
			GetModuleFileNameA(reinterpret_cast<HMODULE>(uiBase), cpBuf, MAX_PATH);
			return PathGetFileName_Ptr(cpBuf);
		}

		size_t GetModuleNameViaBase_Ptr(wchar_t* wpBuf, uintptr_t uiBase)
		{
			GetModuleFileNameW(reinterpret_cast<HMODULE>(uiBase), wpBuf, MAX_PATH);
			return PathGetFileName_Ptr(wpBuf);
		}

		std::string GetModuleNameViaBaseA(uintptr_t uiBase)
		{
			std::string name;
			name.resize(MAX_PATH);
			name.resize(GetModuleNameViaBase_Ptr(&name[0], uiBase));
			return name;
		}

		std::wstring GetModuleNameViaBaseW(uintptr_t uiBase)
		{
			std::wstring name;
			name.resize(MAX_PATH);
			name.resize(GetModuleNameViaBase_Ptr(&name[0], uiBase));
