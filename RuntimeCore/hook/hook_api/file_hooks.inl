		...
		bool HookFileAPIs()
		{
			...
			bool failedCreateFileA = DetourAttachFunc(&rawCreateFileA_Patch, newCreateFileA_Patch);
			bool failedCreateFileW = DetourAttachFunc(&rawCreateFileW_Patch, newCreateFileW_Patch);
			bool failedGetFileAttributesA = DetourAttachFunc(&rawGetFileAttributesA_Patch, newGetFileAttributesA_Patch);
			bool failedGetFileAttributesW = DetourAttachFunc(&rawGetFileAttributesW_Patch, newGetFileAttributesW_Patch);
			bool failedGetFileAttributesExA = DetourAttachFunc(&rawGetFileAttributesExA_Patch, newGetFileAttributesExA_Patch);
			bool failedGetFileAttributesExW = DetourAttachFunc(&rawGetFileAttributesExW_Patch, newGetFileAttributesExW_Patch);
			bool failedReadFile = DetourAttachFunc(&rawReadFile_Patch, newReadFile_Patch);
			bool failedSetFilePointer = DetourAttachFunc(&rawSetFilePointer_Patch, newSetFilePointer_Patch);
			bool failedSetFilePointerEx = DetourAttachFunc(&rawSetFilePointerEx_Patch, newSetFilePointerEx_Patch);
			bool failedGetFileSize = DetourAttachFunc(&rawGetFileSize_Patch, newGetFileSize_Patch);
			bool failedGetFileSizeEx = DetourAttachFunc(&rawGetFileSizeEx_Patch, newGetFileSizeEx_Patch);
			bool failedGetFileInformationByHandle = DetourAttachFunc(&rawGetFileInformationByHandle_Patch, newGetFileInformationByHandle_Patch);
			bool failedGetFileType = DetourAttachFunc(&rawGetFileType_Patch, newGetFileType_Patch);
			bool failedCloseHandle = DetourAttachFunc(&rawCloseHandle_Patch, newCloseHandle_Patch);
			bool failedCreateFileMappingW = DetourAttachFunc(&rawCreateFileMappingW_Patch, newCreateFileMappingW_Patch);
			bool failedCreateFileMappingA = DetourAttachFunc(&rawCreateFileMappingA_Patch, newCreateFileMappingA_Patch);
			bool failedFindFirstFileA = DetourAttachFunc(&rawFindFirstFileA_Patch, newFindFirstFileA_Patch);
			bool failedFindFirstFileW = DetourAttachFunc(&rawFindFirstFileW_Patch, newFindFirstFileW_Patch);
			bool failedFindNextFileA = DetourAttachFunc(&rawFindNextFileA_Patch, newFindNextFileA_Patch);
			bool failedFindNextFileW = DetourAttachFunc(&rawFindNextFileW_Patch, newFindNextFileW_Patch);
			bool failedFindClose = DetourAttachFunc(&rawFindClose_Patch, newFindClose_Patch);
		...
		{
			bool hasFailed = false;
			bool failedCreateFileA = rawCreateFileA_Patch == CreateFileA ? false : DetourDetachFunc(&rawCreateFileA_Patch, newCreateFileA_Patch);
			bool failedCreateFileW = rawCreateFileW_Patch == CreateFileW ? false : DetourDetachFunc(&rawCreateFileW_Patch, newCreateFileW_Patch);
			bool failedGetFileAttributesA = rawGetFileAttributesA_Patch == GetFileAttributesA ? false : DetourDetachFunc(&rawGetFileAttributesA_Patch, newGetFileAttributesA_Patch);
			bool failedGetFileAttributesW = rawGetFileAttributesW_Patch == GetFileAttributesW ? false : DetourDetachFunc(&rawGetFileAttributesW_Patch, newGetFileAttributesW_Patch);
			bool failedGetFileAttributesExA = rawGetFileAttributesExA_Patch == GetFileAttributesExA ? false : DetourDetachFunc(&rawGetFileAttributesExA_Patch, newGetFileAttributesExA_Patch);
			bool failedGetFileAttributesExW = rawGetFileAttributesExW_Patch == GetFileAttributesExW ? false : DetourDetachFunc(&rawGetFileAttributesExW_Patch, newGetFileAttributesExW_Patch);
			bool failedReadFile = rawReadFile_Patch == ReadFile ? false : DetourDetachFunc(&rawReadFile_Patch, newReadFile_Patch);
			bool failedSetFilePointer = rawSetFilePointer_Patch == SetFilePointer ? false : DetourDetachFunc(&rawSetFilePointer_Patch, newSetFilePointer_Patch);
			bool failedSetFilePointerEx = rawSetFilePointerEx_Patch == SetFilePointerEx ? false : DetourDetachFunc(&rawSetFilePointerEx_Patch, newSetFilePointerEx_Patch);
			bool failedGetFileSize = rawGetFileSize_Patch == GetFileSize ? false : DetourDetachFunc(&rawGetFileSize_Patch, newGetFileSize_Patch);
			bool failedGetFileSizeEx = rawGetFileSizeEx_Patch == GetFileSizeEx ? false : DetourDetachFunc(&rawGetFileSizeEx_Patch, newGetFileSizeEx_Patch);
			bool failedGetFileInformationByHandle = rawGetFileInformationByHandle_Patch == GetFileInformationByHandle ? false : DetourDetachFunc(&rawGetFileInformationByHandle_Patch, newGetFileInformationByHandle_Patch);
			bool failedGetFileType = rawGetFileType_Patch == GetFileType ? false : DetourDetachFunc(&rawGetFileType_Patch, newGetFileType_Patch);
			bool failedCloseHandle = rawCloseHandle_Patch == CloseHandle ? false : DetourDetachFunc(&rawCloseHandle_Patch, newCloseHandle_Patch);
			bool failedCreateFileMappingW = rawCreateFileMappingW_Patch == CreateFileMappingW ? false : DetourDetachFunc(&rawCreateFileMappingW_Patch, newCreateFileMappingW_Patch);
			bool failedCreateFileMappingA = rawCreateFileMappingA_Patch == CreateFileMappingA ? false : DetourDetachFunc(&rawCreateFileMappingA_Patch, newCreateFileMappingA_Patch);
			bool failedFindFirstFileA = rawFindFirstFileA_Patch == FindFirstFileA ? false : DetourDetachFunc(&rawFindFirstFileA_Patch, newFindFirstFileA_Patch);
			bool failedFindFirstFileW = rawFindFirstFileW_Patch == FindFirstFileW ? false : DetourDetachFunc(&rawFindFirstFileW_Patch, newFindFirstFileW_Patch);
			bool failedFindNextFileA = rawFindNextFileA_Patch == FindNextFileA ? false : DetourDetachFunc(&rawFindNextFileA_Patch, newFindNextFileA_Patch);
			bool failedFindNextFileW = rawFindNextFileW_Patch == FindNextFileW ? false : DetourDetachFunc(&rawFindNextFileW_Patch, newFindNextFileW_Patch);
			bool failedFindClose = rawFindClose_Patch == FindClose ? false : DetourDetachFunc(&rawFindClose_Patch, newFindClose_Patch);
			hasFailed |= failedCreateFileA;
			hasFailed |= failedCreateFileW;
			hasFailed |= failedGetFileAttributesA;
			hasFailed |= failedGetFileAttributesW;
