...
PIMAGE_NT_HEADERS Pe::GetNtHeaders(HMODULE hModule)
{
    ...
    return reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<uintptr_t>(pDosHeader) + pDosHeader->e_lfanew);
}
...
std::vector<Pe::Section> Pe::GetSections(HMODULE hModule)
{
    ...
    {
        Section section = {};
        section.address = static_cast<uintptr_t>(reinterpret_cast<uintptr_t>(hModule) + pSectionHeaders[i].VirtualAddress);
        section.size = pSectionHeaders[i].SizeOfRawData;
